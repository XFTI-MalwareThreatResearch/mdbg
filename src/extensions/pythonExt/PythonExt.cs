...
{
    private static ScriptEngine g_python; // The main python engine.
    ...
    private static ScriptScope g_scope;
    ...
    public static void LoadExtension()
    {
        ...
        g_python = Python.CreateEngine();
        g_scope = g_python.CreateScope();
        g_pythonInteractive = new PythonCommandLine(g_python, g_scope);
        ...
        ICollection<string> searchPaths = g_python.GetSearchPaths();
        searchPaths.Add(Environment.CurrentDirectory);
        //additionally add the path that this module comes out of, it may have certain imports.
        searchPaths.Add(Path.GetDirectoryName(Assembly.GetEntryAssembly().Location));
        if(!searchPaths.Contains(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location)))
        {
            searchPaths.Add(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location));
        }
        g_python.SetSearchPaths(searchPaths);
        ...
        g_scope.SetVariable("CommandBase", DynamicHelpers.GetPythonTypeFromType(typeof(CommandBase)));
        ...
        g_python.Runtime.IO.SetOutput(Console.OpenStandardOutput(), Encoding.UTF8);
        g_python.Runtime.IO.SetErrorOutput(Console.OpenStandardOutput(), Encoding.UTF8);
        g_python.Runtime.IO.SetInput(Console.OpenStandardInput(), Encoding.UTF8);
...
"
    ...
    {
        object o = g_python.Execute(args);
        PrintPythonResult(o);
    }
    ...
    {
        g_python.ExecuteFile(scriptLocation, g_scope);
    ...
    {
        object r = g_python.Execute("dir()", g_scope);
        ...
        foreach (string name in namesList)
        {
            ...
            try
            {
                r = g_python.Execute(string.Format("{0}.__doc__", name), g_scope);
            }
            catch
            {
                r = null;
    ...
    class MDbgPythonCommand : IMDbgCommand
    {
        ...
        {
            g_python.Execute(string.Format("{0}('{1}')", m_funcName, args), g_scope);
        }
        ...
        bool IEquatable<IMDbgCommand>.Equals(IMDbgCommand other)
        {
    ...
    class MDbgStream : Stream
    {
        ...
        {
            throw new NotSupportedException("Read not supported"); // can't read
        }
