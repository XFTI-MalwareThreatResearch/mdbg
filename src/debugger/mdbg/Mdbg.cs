...
namespace Microsoft.Samples.Tools.Mdbg
{
        ...
        private int RunInputLoop()
        {
            ...
            while (m_run && IO.ReadCommand(out input))
            {
                try
                {
                    ...
                    {
                        CommandBase.WriteOutput("Executing command " + input);
                ...
                catch (Exception e)
                {
                    ...
                    CommandBase.WriteOutput(e.ToString());
                    ReportException(e);
                }
            } // end while
            return m_exitCode;
    ...
    public class MDbgIO : IMDbgIO, IMDbgIO2, IMDbgIO3
    {
        ...
        {
            string logFile = this.m_shell.Debugger.Options.LogFile;
            bool useLogFile = !logFile.Equals(string.Empty);
            ...
            else
            {
                ...
                {
                    File.AppendAllText(logFile, text.Substring(0, highlightStart));
                }
                ...
                {
                    File.WriteAllText(logFile, text.Substring(highlightStart, highlighLen));
                }
