...
namespace Microsoft.Samples.Tools.Mdbg
{
    ...
    sealed internal class MdbgCommands : CommandBase
    {
        ...
        public static void RunCmd(string arguments)
        {
            ...
            if (Debugger.Options.HideManagedDebugger)
                Debugger.ExecuteManagedDebuggerHideOperation(p); //Attempt the hidemanageddebugger operation here.
            bool hasUserEntryBp = p.EnableUserEntryBreakpoint;
            p.Go().WaitOne();
            bool nowHasUserEntryBp = p.EnableUserEntryBreakpoint;

            g_lastSavedRunCommand = arguments;

            Shell.DisplayCurrentLocation();

            if (hasUserEntryBp && !nowHasUserEntryBp)
            {
                //process entrypoint commands now.
                bool epcmdsEnabled = Debugger.Options.EntryPointCommands != null;
                if (epcmdsEnabled)
                {
                    epcmdsEnabled = Debugger.Options.EntryPointCommands.Length != 0;
                }

                if (epcmdsEnabled)
                {
                    foreach (string item in Debugger.Options.EntryPointCommands)
                    {
                        ExecuteCommand(item);
                    }
                }
        ...
        [
         CommandDescription(
           CommandName = "hide",
           MinimumAbbrev = 3,
           ShortHelp = "hides the debugger",
           LongHelp = "Usage: hide\n"
         )
        ]
        public static void HideCmd(string arguments)
        {
            CommandBase.WriteOutput("Instructing the debugger to hide itself.");
            Debugger.Options.HideManagedDebugger = true;
        }
        ...
        [
         CommandDescription(
           CommandName = "cctor",
           MinimumAbbrev = 1,
           ShortHelp = "Cctor entrypoint mode - upon module load, breakpoints will be set on all cctors.",
           LongHelp = "Usage: cctor\n Sets breakpoints on all cctor methods upon module load."
         )
        ]
        public static void CctorCmd(string arguments)
        {
            bool orig = Debugger.Options.BreakOnModuleCctor;
            if (orig)
            {
                CommandBase.WriteOutput("Breaking on module EntryPoint");
                Debugger.Options.BreakOnModuleCctor = false;
            }
            else
            {
                CommandBase.WriteOutput("Breaking on module cctor");
                Debugger.Options.BreakOnModuleCctor = true;
            }
        }
        ...
        [
         CommandDescription(
           CommandName = "dumpexe",
           MinimumAbbrev = 1,
           ShortHelp = "Dumps the currently executing binary from memory into a file",
           LongHelp = "Usage: dumpexe <filename>\n Dumps the currently executing binary from memory into a file"
         )
        ]
        public static void DumpExeCmd(string arguments)
        {
            string filename = arguments;
            //first find the binary we are executing
            MDbgProcess current = Debugger.Processes.Active;
            MDbgFrame currentStackFrame = current.Threads.Active.CurrentFrame;
            MDbgFunction currentFunction = currentStackFrame.Function;
            MDbgModule currentModule = currentFunction.Module;
            long baseAddr = currentModule.CorModule.BaseAddress;
            byte[] dumped_pe = PEReader.ReadFullPeFromProcess(baseAddr, current.CorProcess);
            if (dumped_pe != null)
            {
                File.WriteAllBytes(filename, dumped_pe);
                CommandBase.WriteOutput("Dumped PE to file.");
            }
            else
            {
                CommandBase.WriteOutput("Could not dump pe.");
            }
        }
        ...
        [
         CommandDescription(
           CommandName = "logfile",
           MinimumAbbrev = 1,
           ShortHelp = "Sets a file to log all output to.",
           LongHelp = "Usage: logfile <filepath>\n   Debugger will log all output to <filepath>."
         )
        ]
        public static void LogFileCmd(string arguments)
        {
            Debugger.Options.LogFile = arguments;
        }
        [
         CommandDescription(
           CommandName = "epcmds",
           MinimumAbbrev = 1,
           ShortHelp = "Tells the debugger to execute each line in a file as a debugger command once the entrypoint is hit.",
           LongHelp = "Usage: epcmds <filepath>\n    Debugger will execute all lines in <filepath> once the entrypoint is hit as a debugger command."
         )
        ]
        public static void EpCmdsCmd(string arguments)
        {
            if (File.Exists(arguments))
            {
                Debugger.Options.EntryPointCommands = File.ReadAllLines(arguments);
            }
        }
            ...
            ISequencePointResolver IBreakpointParser.ParseFunctionBreakpoint(string arguments)
            {
                ...
                if(arguments.StartsWith("&"))              
                {
                    if (arguments.Contains("_"))
                    {
                        ...
                        //new type <Full Name>_Offset
                        // string[] args = arguments.Split('_');
                        int index = arguments.LastIndexOf('_');
                        string fullMethodName = arguments.Substring(0, index); //this is the full name
                        string[] mNameArgs = fullMethodName.Split(' ');
                        Debug.Assert(mNameArgs.Length == 2);
                        string noReturnName = mNameArgs[1];
                        //So here we should have the method name minus the return type.  Get the declaring type.
                        mNameArgs = noReturnName.Split("::".ToCharArray());
                        Debug.Assert(mNameArgs.Length == 2);
                        string typeFullName = mNameArgs[0];
                        string ilOffset = arguments.Substring(index + 1); //this is going to be the IL offset.
                        string methodName = fullMethodName;
                        int methodOffset = Convert.ToInt32(ilOffset);
                        MDbgProcess p = Debugger.Processes.Active;
                        string typeName = typeFullName;
                        //instead of checking for location, check by method name
                        MDbgModule moddef = null;
                        MethodDef mdef = null;
                        foreach(MDbgModule modp in p.Modules)
                        {
                            ModuleDefMD dnMod = modp.DnlibModule;
                            if(dnMod == null)
                            {
                                continue;
                            }

                            TypeDef dnTdefActual = null;
                            foreach(TypeDef dnTdef in dnMod.GetTypes())
                            {
                                if(dnTdef.FullName == typeName)
                                {
                                    dnTdefActual = dnTdef;
                                    break;
                                }
                            ...
                            if(dnTdefActual != null)
                            {
                                foreach(MethodDef dnMdef in dnTdefActual.Methods)
                                {
                                    if(dnMdef.FullName == methodName)
                                    {
                                        mdef = dnMdef;
                                        moddef = modp;
                                        break;
                        ...
                        if (moddef != null)
                        {
                            MDbgFunction fp = p.ResolveFunctionToken(moddef, (int)mdef.MDToken.Raw);
                            if (fp != null)
                            {
                                return new BreakpointFunctionToken(fp, methodOffset);
                            }
                            else
                            {
                                CommandBase.WriteOutput("Unable to find function.");
                            }
                        }
                        else
                        {
                            CommandBase.WriteOutput("Could not find parent module.");
                        }
                    }
                }
                ...
                //newly added form - TOKEN_Offset
                if (arguments.StartsWith("0x"))
                {
                    string[] bploc_args = arguments.Split('_');
                    if (bploc_args.Length == 2)
                    {
                        try
                        {
                            int methodToken = Convert.ToInt32(bploc_args[0], 16);
                            int token_type = (methodToken >> 24) & 0xFF;
                            int ilOffset = Convert.ToInt32(bploc_args[1], 16);
                            if (token_type == 0x06)
                            {
                                MDbgProcess p = Debugger.Processes.Active;
                                MDbgModule currentModule = p.Modules.Lookup(Process.GetProcessById(p.CorProcess.Id).MainModule.ModuleName);
                                MDbgFunction fp = p.ResolveFunctionToken(currentModule, methodToken);
                                return new BreakpointFunctionToken(fp, ilOffset);
                            }
                            else if (token_type == 0x0A)
                            {
                                MDbgProcess p = Debugger.Processes.Active;
                                MDbgModule currentModule = p.Modules.Lookup(Process.GetProcessById(p.CorProcess.Id).MainModule.ModuleName);
                                ModuleDefMD mod = currentModule.DnlibModule;
                                if (mod != null)
                                {
                                    uint rid = MDToken.ToRID(methodToken);
                                    MemberRef mref = mod.ResolveMemberRef(rid);
                                    if (mref != null)
                                    {
                                        MethodDef mdef = mref.ResolveMethod();
                                        string moduleName = mdef.Module.Assembly.Name + ".dll";
                                        MDbgModule moddef = null;
                                        foreach (MDbgModule modp in p.Modules)
                                        {
                                            if (modp.CorModule.Name.EndsWith(moduleName))
                                            {
                                                moddef = modp;
                                                break;
                                            }
                                    ...
                                    {
                                        CommandBase.WriteOutput("Could not find memberref.");
                                    }
                                }
                                else
                                {
                                    CommandBase.WriteOutput("Could not set breakpoint due to DnlibModule being null.");
                                }
                            ...
                            {
                                CommandBase.WriteOutput("Invalid prefix.");
                                return null;
                            }
                        }
                        catch
        ...
        [
         CommandDescription(
           CommandName = "dumpbytearrayarg",
           MinimumAbbrev = 3,
           ShortHelp = "Dumps a bytearray argument of the current function to a file.",
           LongHelp = "dumpbytearrayvar [arg no] [filepath]\nThis function is for dumping byte arrays specifically.\nDumps a argument from the current function that is of type byte[] into a file."
         )
        ]
        public static void DumpByteArrayArg(string arguments)
        {
            MDbgFrame frame = Debugger.Processes.Active.Threads.Active.CurrentFrame;
            string[] args = arguments.Split(' ');
            int argNum = Convert.ToInt32(args[0]);
            string[] fp_args = new string[args.Length - 1];
            Array.Copy(args, 1, fp_args, 0, fp_args.Length);
            string filePath = String.Join(" ", fp_args);
            MDbgValue[] funcArgs = frame.Function.GetArguments(frame);
            MDbgValue targetVal = funcArgs[argNum];
            object targetObj = targetVal.GetObjectValue(true);
            if (targetObj is CorArrayValue)
            {
                CorArrayValue arrayObj = (CorArrayValue)targetObj;
                byte[] data = new byte[arrayObj.GetDimensions()[0]];
                for (int i = 0; i < data.Length; i++)
                {
                    CorValue valObj = arrayObj.GetElementAtPosition(i);
                    byte num = (byte)valObj.CastToGenericValue().GetValue();
                    data[i] = num;
                }

                File.WriteAllBytes(filePath, data);
                CommandBase.WriteOutput("Dump saved to " + filePath + ".");
            }
        }

        [
         CommandDescription(
           CommandName = "dumpbytearrayvar",
           MinimumAbbrev = 3,
           ShortHelp = "Dumps a bytearray local variable of the current function to a file.",
           LongHelp = "dumpbytearrayvar [variable id] [filepath]\nThis function is for dumping byte arrays specifically.\nDumps a local variable from the current function that is of type byte[] into a file."
         )
        ]
        public static void DumpByteArrayVar(string arguments)
        {
            MDbgFrame frame = Debugger.Processes.Active.Threads.Active.CurrentFrame;
            string[] args = arguments.Split(' ');
            int argNum = Convert.ToInt32(args[0]);
            ...
            Array.Copy(args, 1, fp_args, 0, fp_args.Length);
            string filePath = String.Join(" ", fp_args);
            MDbgValue[] funcVars = frame.Function.GetActiveLocalVars(frame);
            MDbgValue targetVal = funcVars[argNum];
            object targetObj = targetVal.GetObjectValue(true);
            if (targetObj is CorArrayValue)
            {
                CorArrayValue arrayObj = (CorArrayValue)targetObj;
        ...
        [
         CommandDescription(
           CommandName = "printarg",
           MinimumAbbrev = 3,
           ShortHelp = "Prints an argument within the current function",
           LongHelp = "printarg [arg no]\nPrints a string representation (from ToString) of the specified variable's value"
         )
        ]
        public static void PrintArg(string arguments)
        {
            MDbgFrame frame = Debugger.Processes.Active.Threads.Active.CurrentFrame;
            string[] args = arguments.Split(' ');
            int argNum = Convert.ToInt32(args[0]);
            MDbgValue[] funcArgs = frame.Function.GetArguments(frame);
            MDbgValue targetVal = funcArgs[argNum];
            CommandBase.WriteOutput(targetVal.GetStringValue(true));
        }

        [
         CommandDescription(
           CommandName = "printvar",
           MinimumAbbrev = 3,
           ShortHelp = "Prints an variable within the current function.",
           LongHelp = "printvar [var no]\nPrints a string representation (from ToString) of the specified variable's value."
         )
        ]
        public static void PrintVariable(string arguments)
        {
            MDbgFrame frame = Debugger.Processes.Active.Threads.Active.CurrentFrame;
            string[] args = arguments.Split(' ');
            int argNum = Convert.ToInt32(args[0]);
        ...
        [
         ...
         SecurityPermissionAttribute(System.Security.Permissions.SecurityAction.Demand, Unrestricted = true) // we are loading custom assembly -- need high permissions
        ]
