    ...
    public sealed class MDbgProcess : MarshalByRefObject
    {
        ...
        public bool EnableUserEntryBreakpoint
        {
            ...
            set
            {
                ...
                if (!value && m_userEntryBreakpoint.Count != 0)
                {
                    foreach(MDbgBreakpoint entryBp in m_userEntryBreakpoint)
                    {
                        entryBp.Delete();
                    }
                    m_userEntryBreakpoint.Clear();
                }
        ...
        public MDbgFunction ResolveFunctionToken(MDbgModule mdbgModule, int functionToken)
        {
            return mdbgModule.GetFunction(functionToken);
        }
        ...
        private List<MDbgBreakpoint> m_userEntryBreakpoint = new List<MDbgBreakpoint>();
        private bool m_userEntryBreakpointEnabled = true;

        private bool m_moduleCctorBreakpointEnabled = false;
        ...
        private void LoadModuleEventHandler(Object sender, CorModuleEventArgs e)
        {
            ...
            try
            {
                ...
                if(File.Exists(m.CorModule.Name))
                {

                    ModuleContext context = ModuleDef.CreateModuleContext();
                    m.DnlibModule = ModuleDefMD.Load(m.CorModule.Name, context);
                }
                ...
                if (!m_processAttaching
                    ...
                    && m_userEntryBreakpoint.Count == 0 && this.m_engine.Options.FunctionLogMode == null)
                {
                    SetUserEntryBreakpointInModule(m, this.m_engine.Options.BreakOnModuleCctor);
                }

                if(!m_processAttaching && this.m_engine.Options.FunctionLogMode != null)
                {
                    if(!FunctionLog.IsInitialized())
                    {
                        FunctionLog.Initialize(this.m_engine.Options.FunctionLogMode);
                    }
                    SetupFunctionCallLogBreakpoints(m);
                }
        ...
        private void UpdateModuleSymbolsEventHandler(Object sender, CorUpdateModuleSymbolsEventArgs e)
        {
            ...
            try
            {
                ...
                if (e.Module.IsInMemory && !m_processAttaching &&
                    m_userEntryBreakpointEnabled && m_userEntryBreakpoint.Count == 0 && this.m_engine.Options.FunctionLogMode == null)
                {
                    SetUserEntryBreakpointInModule(m, this.m_engine.Options.BreakOnModuleCctor);
                }
        ...
        private bool SetupFunctionCallLogBreakpoints(MDbgModule m)
        {
            if(m.DnlibModule == null)
            {
                return false;
            }

            if(m.DnlibModule.EntryPoint == null)
            {
                return false;
            }
            bool ok = true;
            foreach(TypeDef tdef in m.DnlibModule.GetTypes())
            {
                if(tdef.IsImport)
                {
                    continue;
                ...
                foreach(MethodDef mdef in tdef.Methods)
                {
                    if(mdef.HasBody)
                    {
                        int st = (int)mdef.MDToken.Raw;
                        MDbgFunction mdbgFunc = m.GetFunction(st);
                        FunctionLogBreakPoint fbp = new FunctionLogBreakPoint(this, mdbgFunc);
                        bool thisOk = fbp.BindToModule(m);
                        if(!thisOk)
                        {
                            Console.WriteLine("Couldnt bind breakpoint for " + mdef.FullName);
                        }
                    }
                }
            }
        ...
        private bool SetUserEntryBreakpointInModule(MDbgModule m, bool setOnCctor)
        {
            bool ok = true;
            if (!setOnCctor) //set the breakpoint on the actual entrypoint
            {
                if (m.SymReader != null)
                {
                    int st = 0;
                    ...
                    if (st != 0)
                    {
                        ...
                        m_userEntryBreakpoint.Add(new UserEntryBreakpoint(this, mfunc));
                        ok = m_userEntryBreakpoint[0].BindToModule(m);
                ...
                else
                {
                    ...
                    //use dnlib to get the token
                    if (File.Exists(m.CorModule.Name))
                    {

                        if (m.DnlibModule != null)
                        {
                            MethodDef ep = m.DnlibModule.EntryPoint;
                            if (ep != null)
                            {
                                int st = (int)ep.MDToken.Raw;
                                MDbgFunction mfunc = m.GetFunction(st);
                                UserEntryBreakpoint ue = new UserEntryBreakpoint(this, mfunc);
                                m_userEntryBreakpoint.Add(ue);
                                ok = ue.BindToModule(m);
            ...
            {
                //use dnlib to set on the module's cctors, theyll all get removed anyway once the first one hits.
                //use dnlib to get the token
                if (File.Exists(m.CorModule.Name))
                {
                    ...
                    if (m.DnlibModule != null)
                    {
                        ...
                        if (ep != null)
                        {
                            //to prevent setting cctors on libraries.
                            foreach (TypeDef tdef in m.DnlibModule.GetTypes())
                            {
                                MethodDef cctorMethod = tdef.FindMethod(".cctor");
                                if (cctorMethod != null)
                                {
                                    int st = (int)cctorMethod.MDToken.Raw;
                                    MDbgFunction mFunc = m.GetFunction(st);
                                    UserEntryBreakpoint newBp = new UserEntryBreakpoint(this, mFunc);
                                    m_userEntryBreakpoint.Add(newBp);
                                    ok = newBp.BindToModule(m);
                                    if (!ok)
                                    {
                                        Trace.WriteLine(string.Format("Failed to set user entry breakpoint at {0}", mFunc.FullName));
                                    }
        ...
        {
            foreach (MDbgBreakpoint bp in m_userEntryBreakpoint)
            {
                bp.Delete();
            }
            m_userEntryBreakpoint.Clear();
        ...
        class FunctionLogBreakPoint: MDbgFunctionBreakpoint
        {
            MDbgProcess m_process;
            internal FunctionLogBreakPoint(MDbgProcess p, MDbgFunction mfunc)
                : base(null, new BreakpointFunctionToken(mfunc, 0))
            {
                m_process = p;
            }
            ...
            public override object OnHitHandler(CustomBreakpointEventArgs e)
            {
                ...
                // Chain to base handlers.
                //object result = base.OnHitHandler(e);
                //For FunctionLogBreakPoints, dont chain to base handlers etc etc, just go.
                BreakpointFunctionToken bpl = (BreakpointFunctionToken)this.Location;
                MDbgFunction mfunc = bpl.GetFunction();
                FunctionLog.WriteString("Hit Method " + mfunc.FullName + ":0x" + mfunc.CorFunction.Token.ToString("X"));
                //m_process.Go();
                //.WaitOne();
                //Debugger.Processes.Active.Go().WaitOne();
                return new BreakpointHitStopReason(this, true);
            }
            public override string ToString()
            {
                return base.ToString() + "(function log bp)";
            }
        }
